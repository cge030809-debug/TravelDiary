"""SQLite storage for trips, locations, photos, diaries, and feedback."""
...
def list_trips_with_diaries() -> list[dict]:
    ...
    for row in rows:
        trip_id = row["trip_id"]
        has_diary = row["diary_json"] is not None
        if not has_diary and not get_locations(trip_id) and not get_photos(trip_id):
            continue
        trips.append(
            {
                "trip_id": trip_id,
                "meta": json.loads(row["meta_json"]) if row["meta_json"] else {},
                "diary": _load_diary(row["diary_json"]) if has_diary else None,
            }
        )
    return trips
